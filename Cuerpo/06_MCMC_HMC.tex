...
\end{equation}
 ...
 Esta distribución canónica es particularmente útil porque \textit{cualquier} distribución de probabilidad puede ser expresada de esta manera. En efecto, consideremos que tenemos un vector de variables aleatorias $s$, cuya distribución de probabilidad es $f(s)$. Esta puede considerarse una distribución canónica como \eqref{eq:Gibbs_Distr_Can} si suponemos que $s$ representa los microestados de un sistema físico ficticio con temperatura $T$ y constante de normalización $Z$ cuya función de energía esté dada por $H(\phi) = -T\,ln\left[Z\,f(s)\right]$. Por construcción, podríamos decir que la temperatura constante es $T=1$ y eliminarla de las ecuaciones.\\
 
 De manera particular, consideremos un vector de parámetros $\phi$ y datos $y$ en el contexto de inferencia bayesiana. Veamos qué sucede si definimos la temperatura como $T=1$ y la función de energía como el producto de inicial por verosimilitud, 
 
\begin{equation*}
 H(\phi|y) = -ln\left[f(y|\phi)f(\phi)\right].
\end{equation*}  
 
 La constante de normalización $Z$ estaría dada por la integral sobre todos los microestados de la distribución canónica sin normalizar: 
 
\begin{equation*}
 Z = \int\limits_\Phi exp\left\lbrace-H(\phi|y)\right\rbrace d\phi = \int\limits_\Phi f(y|\phi)f(\phi)d\phi = f(y)
\end{equation*}