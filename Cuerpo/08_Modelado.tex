...
Consideremos entonces, para cada variable explicativa con $l_m$ categorías poblacionales, modelos de la siguiente forma:
...
\begin{figure}[h]
	...
	\caption{Mapa de predicciones medias del porcentaje bruto de votos obtenido por Marine Le Pen en las presidenciales 2012 mediante el Modelo Nacional por Ocupación Juvenil y mapa de los respectivos errores. Fuente: elaboración propia con la cartografía de Open Street Map.}
...
\intertext{tal que} 
\beta_{d,5} = -\sum\limits_{k = 1}^{4} \beta_{d,k}, \quad & \gamma_{d,8} = -\sum\limits_{k = 1}^{7} \gamma_{d,k}, \nonumber \\
\alpha_d & \sim N(\mu_{\alpha}, \sigma=1), \nonumber \\
\intertext{donde $\forall \, d \in \mathbb{N}_{96}$}
\beta_{d,j} \sim N(\mu_{\beta}, \sigma=1) \quad \forall \, j \in \mathbb{N}_{4}, & \quad \gamma_{d,j} \sim N(\mu_{\gamma}, \sigma=1) \quad \forall \, j \in \mathbb{N}_{7} \nonumber \\
\intertext{y}
\mu_{\alpha} \sim N(-1.7,\sigma=0.25), \quad \mu_{\beta} &\sim N(0,\sigma=0.5) \quad \text{y} \quad \mu_{\gamma} \sim N(0,\sigma=0.5). \nonumber
\end{align}
...
\subsection{Evolución de los efectos}

La variable de escolaridad era la que el ordenamiento preliminar sugería que podría ser la más explicativa. Por ello el modelo 0 comienza con solo esa variable y, progresivamente, fui agregando el resto de variables. Al ir construyendo un modelo más complejo podemos ver cómo se van comportando los efectos estimados para las distintas categorías de escolaridad con respecto a distintos criterios. En la \textbf{Figura \ref{fig:Evol_Efectos_Escolaridad}} observamos cómo evolucionaron la estimación puntual de los efectos para cada departamento, la incertidumbre sobre dicha estimación reflejada en la longitud de los intervalos centrales al 95\%, así como el número de departamentos con efectos significativos positivos y negativos.\\ 

\clearpage
\begin{sidewaysfigure}[h]
	\centering
	\includegraphics[width = 0.95\textwidth]{Figs/Efectos/Evol_Efectos_Escolaridad}
	\caption{Evolución de los efectos de la escolaridad por departamento a través de los distintos modelos. Cada línea gris representa uno de los 96 departamentos. En la fila superior la referencia naranja es un efecto nulo en 0. En la fila de en medio, las líneas verdes representan la longitud promedio a través de los departamentos. En la fila inferior en rosa tenemos efectos negativos y en azul efectos positivos. Fuente: elaboración propia.}
	\label{fig:Evol_Efectos_Escolaridad}
\end{sidewaysfigure}
\clearpage

Vemos por ejemplo que en la mayoría de los departamentos, al agregar las categorías socioprofesionales y, sobre todo, los grupos de edad en el modelo B, los efectos de las personas que todavía estudian se contraen hacia el 0, incrementando su incertidumbre y reduciendo fuertemente la cantidad de departamentos con efectos significativo. Por el contrario, en el caso de las personas sin escolaridad el efecto se va convirtiendo en positivo aumentando el número de departamentos significativos. Si en el modelo 0 había incluso más departamentos con efecto significativo negativo que positivo, para le modelo H ya hay prácticamente puros efectos significativos positivos llegando a superar el número de efectos de este tipo para cualquier otra categoría. Por ejemplo, las personas con preparatoria o equivalente se mantienen con un efecto positivo pero la reducción en el número de departamentos significativos es clara. La categoría aparentemente más significativa sería las personas que tienen diplomas universitarios. Si bien sus coeficientes se van contrayendo hacia el 0 al ir agregando variables, aumentan algo su incertidumbre y, por consiguiente, disminuyen el número de departamentos con efectos significativos, es claro que el número de efectos significativos se mantiene alto siendo 72 de los 96 departamentos.\\

Podemos observar el mismo diagnóstico para las siguientes variables a partir del modelo en el que fueron introducidas. En el caso de las categorías socioprofesionales en la \textbf{Figura \ref{fig:Evol_Efectos_Cat_Socioprof}} vemos que la incertidumbre parece no aumentar mucho conforme se agregan variables, salvo para el caso de los retirados y otras personas sin actividad. Estas son categorías que podríamos llamar ruidosas en el sentido de que observamos mayor incertidumbre y mayor variabilidad tanto en los estimadores puntuales como en el número de efectos significativos. Asimismo, observamos que la categoría más significativa podría ser la de obreros que salvo al agregar edad, mantiene prácticamente el mismo número de departamentos con efecto significativo positivo.\\ 
	
\clearpage
\begin{sidewaysfigure}[h]
	\centering
	\includegraphics[width = 0.95\textwidth]{Figs/Efectos/Evol_Efectos_Cat_Socioprof}
	\caption{Evolución de los efectos de las categorías socioprofesionales por departamento a través de los distintos modelos. Cada línea gris representa uno de los 96 departamentos. En la fila superior la referencia naranja es un efecto nulo en 0. En la fila de en medio, las líneas verdes representan la longitud promedio a través de los departamentos. En la fila inferior en rosa tenemos efectos negativos y en azul efectos positivos. Fuente: elaboración propia.}
	\label{fig:Evol_Efectos_Cat_Socioprof}
\end{sidewaysfigure}
\clearpage
...
\begin{sidewaysfigure}[h]
	\centering
	\includegraphics[width = 0.95\textwidth]{Figs/Efectos/Evol_Efectos_Edad}
	\caption{Evolución de los efectos de los grupos de edad por departamento a través de los distintos modelos. Cada línea gris representa uno de los 96 departamentos. En la fila superior la referencia naranja es un efecto nulo en 0. En la fila de en medio, las líneas verdes representan la longitud promedio a través de los departamentos. En la fila inferior en rosa tenemos efectos negativos y en azul efectos positivos. Fuente: elaboración propia.}
	\label{fig:Evol_Efectos_Edad}
\end{sidewaysfigure}
\clearpage

En el caso de las edades--- \textbf{Figura \ref{fig:Evol_Efectos_Edad}}--- tenemos distintos niveles de incertidumbre. Mientras hay departamentos con longitudes prácticamente nulas en los intervalos, en la categoría de 65 años o más observamos un departamento con una longitud de casi 6pp. Sin embargo, no parece incrementarse o reducirse tanto dicho nivel de incertidumbre conforme se agregan variables. No obstante, los efectos parecen irse concentrando hacia el 0 y sí se van reduciendo el número de efectos significativos. Podríamos decir que el grupo de los menores de edad tiene un efecto positivo mientras que el de los jóvenes de 18 a 24 años y los adultos mayores de 65 tendrían efectos negativos. Aquí observamos una primera explicación de por qué las categorías socioprofesionales ruidosas eran los retirados y las personas sin actividad. Existe una correlación alta entre estas categorías y los grupos de edad. Este es un punto a notar hacia el futuro si se quisiera mejorar o modificar el modelo completo H.\\

Finalmente, para las categorías dicotómicas a excepción de la (des)ocupación de 55 a 64--- que no tuvo evolución al estar presente solo en el modelo completo H---, observamos poca variación en los tres diagnósticos, salvo por algunos departamentos atípicos. La prescencia de inmigrantes tiene un buen número de departamentos significativos con efecto negativo mientras que el resto de categorías parecerían mezclar efectos en ambos sentidos. Una vez observada esta evolución de los efectos al ir construyendo el modelo H, podemos analizar con mayor detalle este último modelo.\\ 

\clearpage
\begin{sidewaysfigure}[h]
	\centering
	\includegraphics[width = 0.95\textwidth]{Figs/Efectos/Evol_Efectos_Dicotom}
	\caption{Evolución de los efectos del resto de variables presentes en al menos dos modelos por departamento. Cada línea gris representa uno de los 96 departamentos. En la fila superior la referencia naranja es un efecto nulo en 0. En la fila de en medio, las líneas verdes representan la longitud promedio a través de los departamentos. En la fila inferior en rosa tenemos efectos negativos y en azul efectos positivos. Fuente: elaboración propia.}
	\label{fig:Evol_Efectos_Dicotom}
\end{sidewaysfigure}
\clearpage

\subsection{Escolaridad}

Comenzamos el análisis de los efectos predichos por el modelo completo por la variable de escolaridad. En la \textbf{Figura \ref{fig:Mapa_Efectos_Escolaridad}} podemos observar el mapa de estimadores puntuales para los efectos departamentales por categorías de escolaridad bajo el modelo H. En cada departamento la intensidad de color refleja la magnitud del efecto mediano; en tonos rojos tenemos efectos negativos y en tonos azules efectos positivos.\\
